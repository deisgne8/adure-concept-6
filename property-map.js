...
function refreshMarkers(){
  ...
  items.forEach(p=>{
    const marker=window.L.marker(propertyLocations[p.id],{
      ...
      title:p.title,alt:'Show '+p.title,keyboard:true
    }).addTo(layer).on('click',()=>select(p.id,true));
...
function select(id,move){
  ...
  selectedId=p.id;preview.hidden=false;
...
list.addEventListener('click',event=>{
  ...
  if(button)select(button.dataset.mapProperty,true);
